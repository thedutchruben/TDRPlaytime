            ...
            if(s.contains("%REWARD_COMMAND%")){
                if(milestone.getCommands().size() == 0){
                    ...
                    for (String command : milestone.getCommands()) {
                        ...
                        textComponent.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/milestone removeCommandFromMilestone " + milestone.getMilestoneName() + " \"" + command + "\""));
                ...
                if(milestone.getItemStacks().size() == 0){
                    ...
                    for (Map<String, Object> itemStack : milestone.getItemStacks()) {
                        ...
                        TextComponent textComponent = new TextComponent("     " +name + " x " + itemStack1.getAmount());
                        textComponent.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponent[]{new TextComponent(ChatColor.DARK_RED + "Click remove the item")}));

                        textComponent.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/milestone removeItemFromMilestone " + milestone.getMilestoneName() + " " + name));
                        sender.spigot().sendMessage(textComponent);
                    }
